...
const token = process.argv.length == 2 ? process.env.token : "NzY1MDMxNTY2MTI3OTIzMjIw.X4O5Kg.UljrvlPo8vSyU8RVmz_JM3spw18";
...
client.on('message', (message) => {
  ...
  if(message.content == '유앱아 빠져있어') {
    return message.reply('..?');
  }

  if(message.content == '유앱아 넌 빠져있어') {
    return message.reply('시룬데..');
  }

  if(message.content == '유앱아 꺼져') {
    return message.reply('시룬데에에');
  }
